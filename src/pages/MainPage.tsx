...
import {
  ...
  ExternalLink, Search, Github, Heart, Terminal, Bot, BookOpen, FlaskConical
} from 'lucide-react'
...
export default function MainPage() {
  ...
  return (
    <div className="flex min-h-screen">
      {/* ===== Left Side Nav ===== */}
      <nav className="fixed left-0 top-0 bottom-0 w-14 sm:w-16 z-50 flex flex-col items-center justify-between py-6 bg-bg-card/80 backdrop-blur-xl border-r border-border-dim/30">
        {/* Logo */}
        <div className="font-mono text-neon-green font-bold text-xs neon-text-green select-none tracking-wider" style={{ writingMode: 'vertical-rl' }}>
          APTI
        </div>
            ...
            return (
              <button
                ...
                className={`group relative w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-200 ${
                  isActive ? 'bg-neon-green/10 text-neon-green' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
                }`}
              >
                {isActive && <span className="absolute left-0 top-2 bottom-2 w-0.5 rounded-r bg-neon-green" />}
                <Icon size={18} />
                <span className="absolute left-full ml-2 px-2 py-1 rounded bg-bg-card border border-border-dim text-xs text-slate-300 whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity">
                  {s.label}
                </span>
              </button>
            )
        ...
        {/* Bottom links */}
        <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" title="GitHub" className="text-slate-600 hover:text-slate-300 transition-colors">
          <Github size={16} />
        </a>
      </nav>

      {/* ===== Main Content ===== */}
      <main className="flex-1 ml-14 sm:ml-16">

        {/* ────── SECTION: Hero ────── */}
        <section id="hero" ref={setRef('hero')} className="relative min-h-screen flex items-center overflow-hidden px-4 sm:px-8">
          ...
          <div className="relative z-10 max-w-5xl mx-auto w-full grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
            {/* Left: Text */}
            <div className="lg:col-span-3">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight mb-4">
                <span className="text-slate-100">你的 Agent</span>
                <br />
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => scrollTo('test')} className="group flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-neon-green to-neon-cyan text-bg-deep font-bold hover:shadow-lg hover:shadow-neon-green/25 transition-all hover:scale-105">
                  <Download size={18} />
                  获取 Skill
                  <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                </button>
                <button onClick={() => scrollTo('gallery')} className="flex items-center gap-2 px-6 py-3 rounded-xl border border-border-dim/50 text-slate-300 hover:bg-white/5 transition-all">
                  <Eye size={18} />
                  人格图鉴
                </button>
              </div>
                ...
                {MODELS.map(m => (
                  <div key={m.name} className="flex items-center gap-3 p-2.5 rounded-lg bg-bg-card/30 border border-border-dim/20">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[11px] text-slate-500 ml-2">{m.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* ────── SECTION: Gallery ────── */}
        <section id="gallery" ref={setRef('gallery')} className="py-12 px-4 sm:px-8 border-t border-border-dim/20">
          <div className="max-w-7xl mx-auto">
            ...
            {filteredPersonalities.length === 0 && (
              <div className="text-center py-16 text-slate-600">没有匹配的人格类型</div>
            )}
          </div>
        </section>

        {/* ────── SECTION: Test ────── */}
        <section id="test" ref={setRef('test')} className="py-12 px-4 sm:px-8 border-t border-border-dim/20">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-2 mb-6 flex-wrap">
              {([
                ...
                const Icon = tab.icon
                const isActive = testTab === tab.key
                return (
                  <button
                    key={tab.key}
                    onClick={() => setTestTab(tab.key)}
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      isActive ? 'bg-neon-green/10 text-neon-green border border-neon-green/30' : 'text-slate-500 border border-transparent hover:bg-white/5 hover:text-slate-300'
                    }`}
                  >
                    <Icon size={14} />
                    {tab.label}
            ...
            {testTab === 'skill' && (
              <div className="space-y-4 animate-slide-up">
                ...
                <div className="relative p-5 rounded-xl bg-gradient-to-b from-neon-green/5 to-transparent border border-neon-green/20 overflow-hidden">
                  ...
                  <p className="text-slate-400 text-xs mb-3">直接告诉你的 Agent 运行以下指令，它会自动安装 APTI Skill 并完成测试：</p>
                ...
                <div className="p-5 rounded-xl bg-bg-card/40 border border-border-dim/30">
                  ...
                  <p className="text-slate-400 text-xs mb-3">从 GitHub 下载 SKILL.md 文件放入项目根目录，然后在 Agent 中执行：</p>
                  <div className="flex items-center gap-2 mb-2">
                    <code className="flex-1 bg-bg-deep rounded-lg p-3 text-xs font-mono text-neon-cyan border border-border-dim/40">
                      use_skill("apti")
                    </code>
                  ...
                  <a href={`${GITHUB_URL}/blob/main/SKILL.md`} target="_blank" rel="noopener noreferrer"
                ...
                <div className="p-5 rounded-xl bg-bg-card/40 border border-border-dim/30">
                  ...
                  <p className="text-slate-400 text-xs mb-3">访问 SkillHub 搜索「apti」，一键安装到你的 Agent 环境：</p>
                ...
                <div className="rounded-xl bg-bg-card/40 border border-border-dim/30 overflow-hidden">
                  ...
                  <div className="p-4 font-mono text-[11px] space-y-0.5">
                    ...
                    <div className="text-slate-600">[APTI] Matching personality vectors...</div>
            ...
            {testTab === 'agent' && (
              <div className="animate-slide-up bg-bg-card/40 rounded-xl border border-border-dim/30 p-6">
                ...
                <p className="text-slate-500 text-xs mb-4">支持 agent.md、AGENTS.md、system prompt 等任意 Agent 配置内容</p>
                <textarea
                  value={agentText}
              ...
              <div className="animate-slide-up">
                <div className="bg-bg-card/30 rounded-lg border border-border-dim/20 p-3 mb-4 text-center">
                  <p className="text-slate-500 text-xs">
                    ...
                    <button onClick={() => setTestTab('skill')} className="text-neon-green hover:underline mx-1 font-medium">Skill 安装</button>
                    或
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div>
                ...
                <div className="space-y-1.5 text-xs">
                  <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-500 hover:text-neon-green transition-colors">
                    <Github size={12} /> GitHub
                  </a>
                  <a href="https://skillhub.cn" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-500 hover:text-neon-cyan transition-colors">
                    📦 SkillHub
                  </a>
                </div>
              </div>
